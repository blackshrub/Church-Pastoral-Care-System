...
import { Heart, Users, Hospital, Calendar, AlertTriangle, DollarSign, Bell, Plus, Check, MoreVertical, Phone, Cake } from 'lucide-react';
    ...
    <div className="space-y-8 pb-12 max-w-full">
      ...
      <Tabs defaultValue="today" className="w-full">
        ...
        <TabsContent value="upcoming" className="space-y-4">
          {upcomingTasks.length === 0 ? (
            <Card>
              <CardContent className="p-6 text-center">{t('no_upcoming_tasks')}</CardContent>
            </Card>
          ) : (
            <Card className="card-border-left-teal">
              <CardHeader>
              ...
              <CardContent>
                <div className="space-y-4">
                  {upcomingTasks.map((task, index) => {
                    const typeConfig = {
                      birthday: { icon: '🎂', color: 'amber', bgClass: 'bg-amber-50', borderClass: 'border-amber-200', btnClass: 'bg-amber-500 hover:bg-amber-600', label: t('birthday_today') },
                      grief_support: { icon: '💔', color: 'purple', bgClass: 'bg-purple-50', borderClass: 'border-purple-200', btnClass: 'bg-purple-500 hover:bg-purple-600', label: t('grief_support') },
                      accident_followup: { icon: '🏥', color: 'blue', bgClass: 'bg-blue-50', borderClass: 'border-blue-200', btnClass: 'bg-blue-500 hover:bg-blue-600', label: t('accident_followup_label') },
                      financial_aid: { icon: '💰', color: 'teal', bgClass: 'bg-teal-50', borderClass: 'border-teal-200', btnClass: 'bg-teal-500 hover:bg-teal-600', label: t('financial_aid') }
                    };
                    const config = typeConfig[task.type] || { icon: '📋', color: 'gray', bgClass: 'bg-gray-50', borderClass: 'border-gray-200', btnClass: 'bg-gray-500 hover:bg-gray-600', label: 'Task' };
                    ...
                    return (
                      <div key={index} className={`p-4 ${config.bgClass} rounded-lg border ${config.borderClass} relative hover:shadow-lg transition-all`}>
                        ...
                        <div className="flex items-start gap-3 mb-3">
                          {/* Avatar with colored ring */}
                          <div className={`flex-shrink-0 rounded-full ring-2 ${
                            config.color === 'amber' ? 'ring-amber-400' : 
